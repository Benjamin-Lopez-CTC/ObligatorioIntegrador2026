(() => {
  ...
  async function fetchArrayBufferWithRetry(url, options = {}) {
    const { timeoutMs = 3e4, retries = 2 } = options;
    let lastError;
...
${u}`, c = n.createShaderModule({ code: d, label: e.name });
  ...
  function decodeResults(outputTensor, charactersDictionary, numClassesFromShape) {
        ...
        async run(image, detection, charactersDictionary, strategy = "per-line") {
          ...
          try {
            ...
            switch (strategy) {
              ...
              case "per-box":
              default:
                return runPerBoxStrategy(sourceCanvasForCrop, validBoxes, ctx, (canvas, box, index, total, debugPath, dict) => this.processBox(canvas, box, index, total, debugPath, dict), charactersDictionary);
            }
          } catch (error) {
...
`);
        ...
        async recognizeTextViaContext(cropCanvas, ctx, charactersDictionary) {
          ...
          try {
            ...
            let result = await ctx.runInference(inputTensor);
            let dict = charactersDictionary ?? ctx.options.charactersDictionary ?? [];
            return decodeResults2(result, dict, tensorWidth);
          } finally {
            inputTensor?.dispose();
          }
        }
